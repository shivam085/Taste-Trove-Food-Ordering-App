...
const Cart = () => {
    ...
    return (
        <>
            <div className={`fixed right-0 top-0 w-full lg:w-[20vw] h-full p-5 bg-white mb-3 ${activeCart ? "translate-x-0" : "translate-x-full"
                ...
                <div className='flex justify-between items-center my-3 '>
                    ...
                    <IoMdClose onClick={() => { setActiveCart(!activeCart) }}
                        className='border-2 border-gray-600 text-gray-600 p-1 text-xl rounded-md hover:text-red-300 hover:border-red-300 cursor-pointer' />
                </div>
                {cartItems.length > 0 ?
                    (cartItems.map((food) => {
                ...
                <div className='absolute bottom-0'>
                    ...
                    <h3 className='font-extrabold text-gray-800'>Total Amount : {totalPrice} </h3>
            ...
            <FaShoppingCart onClick={() => setActiveCart(!activeCart)}
                className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-5 right-4 
